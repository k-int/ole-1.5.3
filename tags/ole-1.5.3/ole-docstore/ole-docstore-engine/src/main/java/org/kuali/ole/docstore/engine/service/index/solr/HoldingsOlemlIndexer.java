...
//            holdings.setCreatedOn((String) holdingsSolrDocument.getFieldValue(CREATED_BY));
    ...
    protected void deleteRecordInSolr(SolrServer solrServer, String id) throws IOException, SolrServerException {
        ...
        query = "holdingsIdentifier:" + id + " AND " + "(DocType:bibliographic)";
        ...
        for (SolrDocument bibSolrDocument : solrDocumentList) {
            List<String> instanceIdentifierList = new ArrayList<String>();
            Object holdingsIdentifier = bibSolrDocument.getFieldValue("bibIdentifier");
            if (holdingsIdentifier instanceof List) {
                instanceIdentifierList = (List<String>) bibSolrDocument.getFieldValue("bibIdentifier");
                if (instanceIdentifierList.contains(id)) {
                    instanceIdentifierList.remove(id);
                    bibSolrDocument.setField("id", instanceIdentifierList);
                }
            } else if (holdingsIdentifier instanceof String) {
                String instId = (String) holdingsIdentifier;
                //if (instId.equalsIgnoreCase(id)) {
                    //holdingsIdentifier = "";
                    bibSolrDocument.setField("bibIdentifier", holdingsIdentifier);
                ///}
            }
            new BibMarcIndexer().buildSolrInputDocFromSolrDoc(bibSolrDocument, solrInputDocument);
            solrInputDocumentList.add(solrInputDocument);
        }
        UpdateResponse updateResponse = solrServer.deleteByQuery("id:" + id);
        System.out.print("updateResponse :"+updateResponse);
        indexSolrDocuments(solrInputDocumentList, true);*/
    }
